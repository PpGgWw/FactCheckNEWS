# 최신 업데이트:
1. API 동작 확인을 위한 업데이트:  
> - 프로세서 HTML, JavaScript 삭제.  
> - 결과 HTML 삭제.  
> - API 연결확인 완료.  
> - 프로세서 HTML, JavaScript 삭제.  
> - 결과 HTML 삭제.  
...
> ```
> [API Client] text-generation-webui API로 분석 요청 시작. 제목: '해외 저궤도 위성통신, 국내 상용화 초읽기...과기정통부,  스타링크·원웹 협정 승인...'
> ```
> ```
> [API Client] POST 요청 전송: http://127.0.0.1:5000/v1/chat/completions (Timeout: 300s)
> ```
> Timeout는 모델 실행 환경이 좋지 못한경우 모델이 추론하기 전에 타임아웃되어 최대 5분 대기 합니다.
> ```
> AI 분석 API 응답 (서버 콘솔):
> {
>   "진위": "진짜 뉴스",
>   "근거": "과기정통부, 스타링크·원웹 협정 승인",
>   "분석": "국내 저궤도 위성통신의 국내 상용화가 사실상 마지막 관문을 넘어섰다."
> }
> ```
2. 
...
> 3. [실행 방법](#-프로젝트-실행-가이드)
> 4. [모델 정보](#-ai-모델-상세-정보)
> 5. [사용 라이브러리](#3-사용-라이브러리)
> 6. [동작 순서 및 방식](#4-동작-순서-및-방식)
> 7. [주요 기술](#-주요-기술-개념-쉽게-이해하기-ai-설명)
---
# 다이어그램
