...
- **AI 응답 포맷팅 문법 추가**
  - `<br>` 태그: 줄바꿈 (예: `첫 줄<br>둘째 줄`)
  - `**텍스트**`: 강조/굵게
  - `## 제목`: 섹션 제목
  - `- 항목` / `1. 항목`: 리스트
  - AI가 위 문법을 사용하여 가독성 높은 응답 생성

- **보안 및 렌더링**
  - `renderMarkdown()` 함수로 XSS 공격 방지 (HTML 이스케이프)
  - 마크다운 문법을 HTML로 안전하게 변환
  - 사용자 입력 및 AI 응답 모두 안전하게 렌더링
...
- **자세히 보기 UX 업그레이드**
  - 패널이 확장된 상태에선 전체 패널을 덮는 상세 뷰 레이어로 결과 표시
  - 상단 뒤로가기 버튼 및 ESC 키 지원으로 빠른 원상 복귀
  - 패널 축소 상태에서는 기존 팝업 모달을 유지해 상황별 일관된 동작 제공
